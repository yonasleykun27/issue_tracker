...
export class EmailService {
  ...
  async sendStatusChangeEmail({
    ...
    const statusConfig: Record<
      string,
      { subject: string; color: string; headline: string; body: string }
    > = {
      IN_PROGRESS: {
  ...
  async sendOtpEmail({ to, otp }: { to: string; otp: string }) {
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  async sendPasswordResetEmail({ to, otp }: { to: string; otp: string }) {
    const html = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="font-family: sans-serif; padding: 20px;">
        ...
        <h3 style="background: #f0f0f0; padding: 15px; text-align: center; font-size: 24px; letter-spacing: 4px;">${otp}</h3>
